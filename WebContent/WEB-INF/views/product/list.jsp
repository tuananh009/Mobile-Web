...
<jsp:include page="../layout/head.jsp">
	<jsp:param name="titlePage" value="Page B" />
</jsp:include>
	...
	<div class="col-sm-3" style="padding: 5px 5px 5px 5px">
		<div class="thumbnail">
			<a href="${rootpath}product/detail/${p.id}"> <c:choose>
					<c:when test="${p.discount >= 0}">
						<p
							...
							<span
								...
								<f:formatNumber value="${discount}" type="percent" />
							</span>
						</p>
					</c:when>
					<c:otherwise>
				...
				class="estore-prod"
				src="${pageContext.request.contextPath}/<c:url value="static"/>/images/products/${p.image}"
				title="${p.name}">
			</a>
			<div class="caption">
				...
				<p
					...
					<c:choose>
						<c:when test="${p.discount <= 0}">
							<span style="text-align: left; font-weight: bold; color: #474c51">
					...
					<c:choose>
						<c:when test="${p.discount >0}">
							<span style="margin-left: 5px; font-weight: bold; color: #474c51">
